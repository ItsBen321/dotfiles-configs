param(
	[string[]]$Query,
	[string]$VaultPath,
	[string]$VaultName = "Gamedev Codex Wiki",
	[switch]$List,
	[switch]$Overview,
	[switch]$PrintPath,
	[switch]$UseRg
...
function Resolve-GameDesignVaultPath {
	param(
		[string]$ExplicitPath,
		[string]$Name
	)
	...
	if ($ExplicitPath) {
		...
		throw "Vault path was provided but is not a valid game-design Obsidian vault: $ExplicitPath"
	}

	$envCandidates = @(
		$env:GAME_DESIGN_VAULT,
		$env:GAME_DESIGN_VAULT_PATH,
		$env:CODEX_GAME_DESIGN_VAULT
	)
	...
	if ($HOME) {
		...
		$configCandidates += Join-Path $codexDir "game-design-vault.txt"
		$configCandidates += Join-Path $codexDir "game-design-vault.path"
	}
	...
	throw "Could not find the game-design Obsidian vault. Pass -VaultPath, set GAME_DESIGN_VAULT, or add the vault to Obsidian. Candidates tried:`n- $attemptedText"
}

$VaultPath = Resolve-GameDesignVaultPath -ExplicitPath $VaultPath -Name $VaultName
...
$targets = @()
$targets += @{ Label = $VaultName; Path = $VaultPath }
...
if ($List) {
	foreach ($target in $targets) {
		Write-Output "## Vault: $($target.Label)"
		Get-MarkdownFiles -Path $target.Path | ForEach-Object { $_.FullName }
		Write-Output ""
	}
	exit 0
}
...
if ($Overview) {
	foreach ($target in $targets) {
		...
		Get-MarkdownFiles -Path $target.Path |
			Sort-Object FullName |
			ForEach-Object {
				$note = Read-NoteOverview -File $_ -Root $target.Path
...
foreach ($target in $targets) {
	...
	foreach ($term in $normalizedQuery) {
		...
		if ($useRg) {
			try {
				& $rg.Source --line-number --ignore-case --hidden --glob "*.md" --glob "!.obsidian/**" --fixed-strings -- $term $target.Path
			} catch {
				Search-WithSelectString -Term $term -Path $target.Path
			}
		} else {
			Search-WithSelectString -Term $term -Path $target.Path
		}
